...
def trend_candidate(fast: int, slow: int) -> Candidate:
    ...
    return Candidate(f"trend_ema_{fast}_{slow}", "trend", {"fast": fast, "slow": slow}, 2, build)
...
def momentum_candidate(period: int, threshold: float) -> Candidate:
    ...
    return Candidate(f"momentum_rsi_{period}_{int(threshold)}", "momentum", {"period": period, "threshold": threshold}, 2, build)
...
def mean_reversion_candidate(period: int, width: float) -> Candidate:
    ...
    return Candidate(f"meanrev_bb_{period}_{width:.1f}", "mean_reversion", {"period": period, "width": width}, 2, build)
...
def breakout_candidate(period: int) -> Candidate:
    ...
    return Candidate(f"breakout_donchian_{period}", "breakout", {"period": period}, 1, build)
...
def volume_breakout_candidate(period: int, relvol: float) -> Candidate:
    ...
    return Candidate(f"volume_breakout_{period}_{relvol:.1f}", "volume", {"period": period, "relvol": relvol}, 2, build)
...
def price_action_candidate(lookback: int, body_ratio: float) -> Candidate:
    ...
    return Candidate(f"price_action_{lookback}_{body_ratio:.1f}", "price_action", {"lookback": lookback, "body_ratio": body_ratio}, 2, build)
