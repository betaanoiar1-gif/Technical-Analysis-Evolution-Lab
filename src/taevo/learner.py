...
class EvolutionLearner:
    """Simple, auditable evolutionary search.

    The learner changes the search distribution from measured evidence. It never sees
    the final holdout while choosing the next generation.
    """
    ...
    def run(self, train: pd.DataFrame, validation: pd.DataFrame, holdout: pd.DataFrame) -> dict:
        ...
        for cycle in range(cycles):
            ...
            for strategy in population[:target]:
                report = validate(strategy, train, validation, holdout, self.cfg, self.rng)
