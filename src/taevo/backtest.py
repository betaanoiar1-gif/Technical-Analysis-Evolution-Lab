...
def run_backtest(
    ...
    pos = signal.reindex(df.index).fillna(0).astype(float).clip(0, 1)
    ...
    years = max((df.index[-1] - df.index[0]).total_seconds() / (365.25 * 86400), 1 / 365.25)
    cagr = float((equity.iloc[-1] / initial_capital) ** (1 / years) - 1)
    vol = float(strategy_ret.std(ddof=0) * np.sqrt(24 * 365))
    sharpe = float(strategy_ret.mean() / strategy_ret.std(ddof=0) * np.sqrt(24 * 365)) if strategy_ret.std(ddof=0) > 0 else 0.0
    metrics = {
        ...
        "exposure": float(executed.mean()),
        "cost_drag": float(cost.sum()),
    }
    return BacktestResult(equity, strategy_ret, executed, metrics)


def buy_and_hold(df: pd.DataFrame, initial_capital: float = 1000.0, fee_bps: float = 10.0) -> dict:
    ...
    return {"total_return": float(equity.iloc[-1] / initial_capital - 1), "final_equity": float(equity.iloc[-1])}
