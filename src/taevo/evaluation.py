...
def score(metrics: dict, complexity: int, min_trades: int = 30, max_drawdown: float = 0.35) -> float:
    ...
    return float(metrics["sharpe"] + 0.50 * metrics["total_return"] + 0.20 * np.log1p(max(metrics["profit_factor"], 0)) - dd_penalty - 0.03 * complexity)


def evaluate_strategy(strategy: Strategy, df: pd.DataFrame, capital: float, fee_bps: float, slippage_bps: float, min_trades: int) -> dict:
...
def robustness(strategy: Strategy, df: pd.DataFrame, capital: float, fee_bps: float, slippage_bps: float, trials: int, rng: np.random.Generator) -> dict:
    ...
    for _ in range(trials):
        ...
        metrics = run_backtest(perturbed, strategy.signal(perturbed), capital, fee_bps, slippage_bps).metrics
        values.append(metrics["sharpe"])
...
def walk_forward(strategy: Strategy, df: pd.DataFrame, capital: float, fee_bps: float, slippage_bps: float, windows: int) -> dict:
    ...
    for i in range(windows):
        ...
        if len(chunk) < 50:
            continue
        m = run_backtest(chunk, strategy.signal(chunk), capital, fee_bps, slippage_bps).metrics
...
def validate(strategy: Strategy, train: pd.DataFrame, validation: pd.DataFrame, holdout: pd.DataFrame, cfg: dict, rng: np.random.Generator) -> dict:
    costs = cfg["costs"]
    vcfg = cfg["validation"]
    capital = float(cfg["capital"]["initial_usd"])
    train_m = evaluate_strategy(strategy, train, capital, costs["fee_bps"], costs["slippage_bps"], vcfg["min_trades"])
    val_m = evaluate_strategy(strategy, validation, capital, costs["fee_bps"], costs["slippage_bps"], vcfg["min_trades"])
    hold_m = evaluate_strategy(strategy, holdout, capital, costs["fee_bps"], costs["slippage_bps"], 0)
    base = buy_and_hold(holdout, capital, costs["fee_bps"])
    ...
    return {"train": train_m, "validation": val_m, "holdout": hold_m, "holdout_buy_hold": base, "robustness": rob, "walk_forward": wf, "rejected": bool(reasons), "rejection_reasons": reasons}
